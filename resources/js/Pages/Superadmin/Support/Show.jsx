...
export default function Show({ ticket }) {
    ...
    const updateStatus = (newStatus) => {
        router.patch(route('superadmin.support.status', ticket.id), { status: newStatus }, { preserveScroll: true });
    };
    ...
    return (
        <AuthenticatedLayout>
            ...
            <div className="max-w-4xl mx-auto w-full">
                <div className="mb-6">
                    <Link href={route('superadmin.support.index')} className="text-gray-400 hover:text-gray-900 font-bold text-sm flex items-center transition-colors">
                    ...
                    <div className="px-8 py-6 border-b border-gray-100 bg-gray-50/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                            ...
                            <p className="text-sm font-bold text-gray-400 mt-1 flex items-center">
                                {ticket.tenant?.name} • Ticket #{ticket.id} • Opened {new Date(ticket.created_at).toLocaleDateString()}
                            </p>
                        </div>
                        ...
                        {ticket.messages.map((msg, idx) => (
                            <div key={idx} className={`flex flex-col max-w-[80%] ${!msg.is_superadmin_reply ? 'mr-auto items-start' : 'ml-auto items-end'}`}>
                                <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1 ml-1">
                                    {!msg.is_superadmin_reply ? msg.user?.name : 'You (Support)'} • {new Date(msg.created_at).toLocaleString()}
                                </span>
                            ...
                            <form onSubmit={submit} className="flex gap-4">
                                <div className="flex-1">
                                    <textarea
                                        ...
                                        className="w-full rounded-2xl border-gray-200 bg-gray-50 px-5 py-3 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 font-medium text-gray-800 transition-all resize-none"
                                ...
                                <button
                                    type="submit"
                                    disabled={processing}
                                    className="bg-blue-600 hover:bg-blue-700 text-white font-black px-6 rounded-2xl shadow-lg shadow-blue-500/30 transition-all disabled:opacity-70 flex items-center shrink-0 h-[64px]"
                                >
